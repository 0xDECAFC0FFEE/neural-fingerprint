...
def train_nn(pred_fun, loss_fun, num_weights, train_smiles, train_raw_targets, train_params, seed=0,
    ...
    train_targets = np.array(train_raw_targets, dtype = bool)
    training_curve = []
    def callback(weights, iter):
        if iter % 10 == 0:
            print "max of weights", np.max(np.abs(weights))
            train_preds = pred_fun(weights, train_smiles[:num_print_examples])
            cur_loss = loss_fun(weights, train_smiles[:num_print_examples], train_targets[:num_print_examples])
            training_curve.append(cur_loss)
            print "Iteration", iter, "loss", cur_loss,\
                  "train accuracy", accuracy(train_preds, train_raw_targets[:num_print_examples]),
            if validation_smiles is not None:
                validation_preds = pred_fun(weights, validation_smiles)
                print "Validation accuracy", iter, ":", accuracy(validation_preds, validation_raw_targets),
...
def main():
    ...
    def run_conv_experiment():
        ...
        loss_fun, pred_fun, conv_parser = \
            build_conv_class_net(conv_arch_params, vanilla_net_params, model_params['L2_reg'])
    ...
    print
    print "Neural test accuracy:", test_loss_neural
