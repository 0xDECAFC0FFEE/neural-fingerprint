...
# skip mcc
    ...
    TP = sum([1 for test, pred in zip(test_y, pred_y)
              if test == pred and pred == 1])
    FP = sum([1 for test, pred in zip(test_y, pred_y)
              if test != pred and pred == 1])
    TN = sum([1 for test, pred in zip(test_y, pred_y)
              if test == pred and pred == 0])
    FN = sum([1 for test, pred in zip(test_y, pred_y)
              if test != pred and pred == 0])
    ...
    if validation_weights != None:
        weighted_log_loss = log_loss(
            test_y, pred_y_proba, sample_weight=validation_weights)
    else:
...
def sampling(arguments, classifier_type, dataset):
    ...
    if neg / pos >= 2:
        ...
        for i in range(pos / neg):
            train_sample = pos_train_X[stop:stop + neg] + \
                neg_train_X, pos_train_Y[stop:stop + neg] + neg_train_Y
            stop = stop + neg
            dataset = (train_sample, validation_dataset)
            results.append(fit_score_classifier(
                arguments, classifier_type, dataset, validation_weights))
...
def cv_layer_1(arguments, classifier_type, dataset, folds):
    ...
    abt = AdaBoostClassifier(clf, n_estimators=2)

    abt.fit(train_X, train_y)

    pred_y = abt.predict_proba(test_X)
...
def log_experiment(results, filename):
    ...
    result_keys_not_in_header = [
        key for key in results[0].keys() if key not in header]
...
def experiment(dataset, classifier_type, classifier_inputs, folds, output_log):
    ...
    for arg_vals in product(*arg_ranges):
        ...
        classifier_argument = {
            arg_name: arg_val for arg_name, arg_val in classifier_argument}
...
def svm_experiment(dataset, output_log):
    ...
    for classifier_inputs in classifier_inputs_list:
        results.extend(experiment(dataset, classifier,
                                  classifier_inputs, folds, output_log))
    return results
...
def dud_experiment():
    ...
    dud_smile_csv_files = ["dud/smiles/%s.csv" %
                           dataset for dataset in dud_datasets]
    make_folder("dud/smiles")
    make_files(dud_smile_csv_files)
    dud_fingerprint_files = ["dud/fingerprints/%s.csv" %
                             dataset for dataset in dud_datasets]
    make_folder("dud/fingerprints")
    make_files(dud_fingerprint_files)
    dud_result_files = ["dud/results/RegvsNN_%s.csv" %
                        dataset for dataset in dud_datasets]
